...
>>>>>>> 26259ec8534b2d94e29983ff8c7e3d810210c758
        return self.team_size

    def set_team_size(self, new_team_size: int):
        try:
<<<<<<< HEAD
            self.db.execute("UPDATE tournament SET team_size = ? WHERE tournament_id = ?", (new_team_size, self.tournament_id))
            self.team_size = new_team_size
            logger.info(f"team_size updated to '{new_team_size}' for Torunement ID {self.tournament_id}")
...
>>>>>>> 26259ec8534b2d94e29983ff8c7e3d810210c758
    ...
    def set_date(self, new_date: int):
        try:
<<<<<<< HEAD
            self.db.execute("UPDATE tournament SET date = ? WHERE tournament_id = ?", (new_date, self.tournament_id))
            self.date = new_date
            logger.info(f"date updated to '{new_date}' for Torunement ID {self.tournament_id}")
        except Exception as e:
    ...
    def set_title(self, new_title: str):
        try:
            self.db.execute("UPDATE tournament SET title = ? WHERE tournament_id = ?", (new_title, self.tournament_id))
            self.title = new_title
            logger.info(f"title updated to '{new_title}' for Torunement ID {self.tournament_id}")
        except Exception as e:
            logger.error(f"Error updating title: {e}")
    ...
    def set_is_official(self, new_is_official: bool):
        try:
            self.db.execute("UPDATE tournament SET is_official = ? WHERE tournament_id = ?", (new_is_official, self.tournament_id))
            self.is_official = new_is_official
            logger.info(f"is_official updated to '{new_is_official}' for Torunement ID {self.tournament_id}")
        except Exception as e:
